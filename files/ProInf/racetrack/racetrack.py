...
def start_race():
	...
	for round in range(max_rounds):
		...
		for car in unfinished_cars:
			...
			log ('<{d}> in {c} car #{n} at {p} with {v}, time spent: {t}'
					.format(n=car.number, d=car.driver_name, c=car.color,
	...
	for i in range (len(results)):
		...
		print ('{rank}: <{dr}> in {c} car #{n}, rounds={r}, distance={d}, time={t}'
			.format(rank=i+1, n=car.number, dr=car.driver_name, c=car.color,
...
try:
	opts, args = getopt.getopt(sys.argv[1:], "dvm:c:r:t:e:", 
		["mapfile=", "cellsize=", "maxrounds=", "timedelay=", "drivertime="])
except getopt.GetoptError:
	print ('usage: racetrack.py -d -v -m <mapfile> -c <cellsize> -r <maxrounds> '
